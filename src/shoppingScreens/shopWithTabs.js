...
export default class shopWithTabs extends Component {
                ...
                return (
                  <TouchableOpacity
                    onPress={() => {
                      ...
                      let newArray = [...this.state.StoreShelvesTop];

                      //Find and apdate the color of the newly selected categoru in the newArray
                      const newCategoryIndex = this.state.StoreShelvesTop.findIndex(
                        (element) => element.CategoryName == item.CategoryName
                      );
                      newArray[newCategoryIndex] = {
                        ...newArray[newCategoryIndex],
                        backgroundColor: "#ed0971",
                      };

                      //Find and apdate the color of the old categoru in the newArray
                      const oldCategoryIndex = this.state.StoreShelvesTop.findIndex(
                        (element) =>
                          element.CategoryName == this.state.currentCategory
                      );
                      newArray[oldCategoryIndex] = {
                        ...newArray[oldCategoryIndex],
                        backgroundColor: "#990849",
                      };

                      //Update StoreShelvesTop state with the newly modified array
                      this.setState({
                        StoreShelvesTop: newArray,
                      });

                      //Fetch data for the newly selected category
                      this.reLoadData(item.CategoryName);
                    }}
                    style={{
                      height: "90%",
                      justifyContent: "center",
